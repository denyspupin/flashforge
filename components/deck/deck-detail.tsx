...
export function DeckDetail({
  ...
  const [editingValues, setEditingValues] = useState<Record<string, { front: string; back: string }>>({})
  ...
  const startEditingCard = (card: Card) => {
    ...
    setEditingValues((prev) => ({
      ...prev,
      [card.id]: { front: card.front, back: card.back },
    }))
  }

  const saveEditingCard = (card: Card) => {
    const values = editingValues[card.id]
    if (!values) return
    updateCardMutation.mutate({
      cardId: card.id,
      cardData: { front: values.front, back: values.back },
    })
  }
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          {sourceLanguage && targetLanguage && (
            <span className="text-muted-foreground inline-flex items-center gap-1.5 font-mono text-[11px] uppercase tracking-widest">
              {sourceLanguage.flag && (
          ...
          <Badge tone={deck.visibility === "public" ? "brand" : "neutral"}>
              ...
              <CardContent className="p-4">
                {editingCard === card.id ? (
                  <div className="flex items-start gap-3">
                    <div className="min-w-0 flex-1">
                      <Input
                        value={editingValues[card.id]?.front ?? ""}
                        onChange={(e) =>
                          setEditingValues((prev) => ({
                            ...prev,
                            [card.id]: {
                      ...
                      />
                      {sourceLanguage && (
                        <p className="text-muted-foreground mt-1 pl-2.5 font-mono text-[10px] uppercase tracking-widest">
                          {sourceLanguage.name}
                        </p>
                      )}
                    </div>
                    <div className="min-w-0 flex-1">
                      <Input
                        value={editingValues[card.id]?.back ?? ""}
                        onChange={(e) =>
                          setEditingValues((prev) => ({
                            ...prev,
                            [card.id]: {
                  ...
                  <div className="flex items-start gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="line-clamp-3 break-words text-2xl font-semibold tracking-tight text-foreground">
                        {card.front}
                      </p>
                      {sourceLanguage && (
      ...
      <AlertDialog open={confirmDeleteOpen} onOpenChange={setConfirmDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This can’t be undone. All cards and study history for this deck
